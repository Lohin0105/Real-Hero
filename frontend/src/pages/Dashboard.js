  ...
  return (
    ...
    >
      ...
      <Dialog open={showNewUserModal} onClose={() => setShowNewUserModal(false)}>
        ...
        <DialogActions>
          <Button onClick={() => setShowNewUserModal(false)}>Later</Button>
          <Button onClick={() => { setShowNewUserModal(false); navigate('/profile'); }} sx={{ background: 'linear-gradient(135deg,#ff2b2b,#b60000)', color: '#fff' }}>
            Profile
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Box sx={{ display: "flex", gap: 4, px: { xs: 2, md: 6 }, pb: 6 }}>
        ...
        <Box sx={{ flex: 1, pt: 3 }}>
          ...
          >
            <Typography variant="h4" sx={{ color: "#ff2b2b", fontWeight: 800, mb: 3 }}>
