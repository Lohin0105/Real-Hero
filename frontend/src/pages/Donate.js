    ...
    >
      ...
      <Box sx={{ display: "flex", gap: 4, px: { xs: 2, md: 6 } }}>
        ...
        <Box sx={{ flex: 1, pt: 3 }}>
            ...
            <Card sx={{ flex: 1, background: "rgba(255,255,255,0.02)", boxShadow: "0 20px 60px rgba(0,0,0,0.5)" }}>
              <CardContent sx={{ display: "flex", justifyContent: "space-between", gap: 2 }}>
                <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                  ...
                  <Box>
                    <Typography sx={{ fontWeight: 700, color: "#fff", fontSize: { xs: "1rem", md: "1.2rem" } }}>
                      {user?.name}
                      {coords && !isMobile && <span style={{ fontSize: '0.8rem', color: '#aaa', marginLeft: '8px' }}>({coords.lat.toFixed(4)}, {coords.lng.toFixed(4)})</span>}
                    </Typography>
                    <Typography variant="caption" sx={{ color: "#aaa", display: { xs: 'none', sm: 'block' } }}>{user?.email}</Typography>
                    <Typography variant="caption" sx={{ color: "#bbb", display: "block", mt: 1, fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>
                ...
                <Box sx={{ textAlign: "right" }}>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, mt: 1 }}>
                    ...
                    }} sx={{ ml: 1, color: '#fff', borderColor: '#666', minHeight: { xs: 48, md: 36 }, fontSize: { xs: '0.75rem', md: '0.875rem' } }}>
          ...
          {/* ACTION BUTTONS */}
          <Box sx={{ display: "flex", gap: 2, alignItems: "center", flexDirection: { xs: 'column', md: 'row' }, width: '100%' }}>
  ...
  return (
    <Dialog open={Boolean(request)} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogActions sx={{ flexDirection: { xs: 'column', sm: 'row' }, gap: 1, p: 2 }}>
        <Button
          onClick={async () => { if (request && request.phone) { try { if (onCreateOffer) onCreateOffer(request, 'call'); } catch (e) { console.warn(e); } } }}
          sx={{
            mr: { xs: 0, sm: 1 },
            minHeight: 48,
        ...
        <Button
          onClick={async () => { if (onCreateOffer && request) { try { onCreateOffer(request, 'navigate'); } catch (e) { console.warn(e); } } }}
          sx={{
            mr: { xs: 0, sm: 1 },
            minHeight: 48,
            width: { xs: '100%', sm: 'auto' }
